...
(() => {
  ...
  // Request adapter: the unchanged controller still calls the same V21/V28 APIs.
  // The selected MIMO controls are injected into those existing requests.
  window.fetch = async (input, init) => {
    ...
    try {
      ...
      return new Response(JSON.stringify({ok:false,error:`Backend returned non-JSON (${response.status} ${response.statusText}): ${message}`}), {status:response.status,statusText:response.statusText,headers:{'Content-Type':'application/json'}});
    }
    return response;
  };
