...
(() => {
  ...
  // Additive request/response adapter. Existing controller calls are unchanged.
    ...
    try {
      ...
      if (originalUrl.includes('/api/lab') && window.__v64MimoState) {
        ...
        if (version === 'V21' || version === 'V28') {
          ...
          if (version === 'V28') {
            u.searchParams.set('beams', String(m.beamSweep ? m.beams : 1));
          }
          requestInput = u.toString();
        }
      }
    } catch (_) {
      // Preserve the original request if the optional adapter cannot modify it.
    }
  ...
  addStyle(`
    ...
    .mimo-result-grid,.phy-result-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}
    .mimo-result-card,.phy-result-card{background:#071c30;border:1px solid #15415f;border-radius:6px;padding:12px}
    .mimo-result-head,.phy-result-head{display:flex;justify-content:space-between;align-items:flex-start;gap:10px;margin-bottom:8px}
    .mimo-result-head strong,.phy-result-head strong{display:block;font-size:13px;color:#e8f4ff}
    .mimo-result-head small,.phy-result-head small{display:block;color:#7891a8;font-size:10px;margin-top:3px}
    .mimo-status,.phy-status{font-size:10px;font-weight:800;white-space:nowrap}
    .mimo-good,.phy-good{color:#00e994}.mimo-bad,.phy-bad{color:#ff5264}
    .mimo-metric,.phy-metric{display:flex;justify-content:space-between;gap:12px;border-bottom:1px solid #0d2b45;padding:6px 0;font-size:11px}
    .mimo-metric span,.phy-metric span{color:#9bb0c3}.mimo-metric b,.phy-metric b{color:#e7f5ff}
    .mimo-note,.phy-note{margin-top:9px;padding:8px;border-left:2px solid #159cff;background:#041522;color:#7f9ab1;font-size:9px;line-height:1.4}
    ...
    #panel-phy .phy-control-row .field,#panel-phy .phy-control-row .actions{margin-top:0}
    #panel-phy .phy-control-row .actions{display:block}
    ...
    @media(max-width:1100px){#panel-phy .phy-control-row{grid-template-columns:repeat(4,minmax(110px,1fr))}#panel-phy .phy-control-row .actions{grid-column:1/-1}#panel-phy .phy-chain-card .pipeline{grid-template-columns:repeat(3,minmax(0,1fr))}#panel-phy .phy-chain-card .stage:nth-child(3n)::after{display:none}}
    @media(max-width:700px){.mimo-result-grid,.phy-result-grid{grid-template-columns:1fr}#panel-phy .phy-control-row{grid-template-columns:1fr 1fr}#panel-phy .phy-control-row .actions{grid-column:1/-1}#panel-phy .phy-chain-card .pipeline{grid-template-columns:1fr 1fr}#panel-phy .phy-chain-card .stage::after{display:none}}
  `);
  ...
  function renderPhyResult(raw) {
    ...
    const p=parts[0].replace(/^.*?NrPuschV20Result\(/s,''), s=(parts[1]||'').replace(/^NrSrsV21Result\(/s,'');
  ...
  function installPhyLayoutAndPresentation() {
    ...
    let attempts=0;
    const timer=setInterval(()=>{if(apply()||++attempts>100)clearInterval(timer)},100);
    let phyAttempts=0;
    const pTimer=setInterval(()=>{
      ...
      if(++phyAttempts>100)clearInterval(pTimer);
    },100);
  }
  ...
  script.src='/radio-v64-fixed.js';
  script.defer=false;
  document.head.appendChild(script);

  const plotScript=document.createElement('script');
  plotScript.src='/radio-v64-plots.js';
  plotScript.defer=false;
