...
object NrV102V110Additive {
    ...
    fun mapResources(
        ...
        nsym: Int = 14,
        layers: Int = 1,
        dmrs: Set<Re> = emptySet(),
        reserved: Set<Re> = emptySet()
    ): Grid {
        require(nsc > 0 && nsym > 0 && layers in 1..8)
        val valid = (0 until nsym).flatMap { l ->
            (0 until layers).flatMap { a -> (0 until nsc).map { k -> Re(k, l, a) } }
        }.toSet()
        val allReserved = dmrs + reserved
        val collisions = allReserved.count { it !in valid }
        val validReserved = allReserved.intersect(valid)
        val data = valid - validReserved
        return Grid(nsc, nsym, validReserved, data, collisions)
    }
    ...
    fun ofdmModulate(freq: Array<NrDmrsMimoV90.Complex>, cp: Int = 4): Ofdm {
        ...
        return Ofdm(
            Array(freq.size + cp) { i -> if (i < cp) body[body.size - cp + i] else body[i - cp] },
            freq.size, cp, 0.0
        )
    }
    ...
    fun applyChannel(x: Array<NrDmrsMimoV90.Complex>, snrDb: Double = 80.0): Channel {
        ...
        val random = Random(1105)
        val sigma = sqrt(variance / 2.0)
        return Channel(Array(x.size) {
            NrDmrsMimoV90.Complex(x[it].re + sigma * gaussian(random), x[it].im + sigma * gaussian(random))
        }, snrDb)
    }
    ...
    fun estimateAndEqualize(
        rx: Array<NrDmrsMimoV90.Complex>,
        ref: Array<NrDmrsMimoV90.Complex>
    ): Equalized {
        ...
        for (i in rx.indices) {
            val d = ref[i]
            val den = d.abs2().coerceAtLeast(1e-12)
            hr += (rx[i].re * d.re + rx[i].im * d.im) / den
            hi += (rx[i].im * d.re - rx[i].re * d.im) / den
        }
        hr /= rx.size
        hi /= rx.size
        val den = (hr * hr + hi * hi).coerceAtLeast(1e-12)
        val equalized = Array(rx.size) {
            val z = rx[it]
            NrDmrsMimoV90.Complex((z.re * hr + z.im * hi) / den, (z.im * hr - z.re * hi) / den)
        }
    ...
    fun softDemodulate(s: Array<NrDmrsMimoV90.Complex>, bitsPerSymbol: Int = 2): SoftBits {
        ...
        return SoftBits(DoubleArray(s.size * bitsPerSymbol).also { a ->
            ...
            for (z in s) {
                a[p++] = 12.0 * z.re
                if (bitsPerSymbol > 1) a[p++] = 12.0 * z.im
                while (p % bitsPerSymbol != 0) a[p++] = 0.0
            }
        })
    }

    /**
     * V110 integration boundary using the existing normative-progressing V87/V84/V86
     * chain. This does not replace those implementations and deliberately reports
     * structural TB-CRC length validation rather than claiming an independent CRC.
     */
    fun integrateLdpc(
        payload: IntArray = IntArray(128) { it and 1 },
        targetCodeRate: Double = 0.5,
        outputBitsPerCodeBlock: Int = 256,
        rv: Int = 0
    ): LdpcIntegration {
        val bg = NrLdpcV82.selectBaseGraph(payload.size, targetCodeRate)
        ...
        val recovered = NrRateMatchingV84.rateRecover(
            llr,
            NrRateMatchingV84.Config(encoded.baseGraph, encoded.liftingSize, rv, rm.size)
        )
        val decoded = NrLdpcCodecV86.decode(recovered, table, encoded.liftingSize, encoded.liftingSet)
        val crcLengthOk = encoded.transportWithCrc.size == payload.size + 16
        return LdpcIntegration(true, recovered.any { it != 0.0 }, decoded.converged || decoded.syndromeWeight == 0, crcLengthOk)
    }

    fun report(): Report {
        val dmrs = (0 until 72 step 2).map { Re(it, 2, 0) }.toSet()
        val grid = mapResources(dmrs = dmrs)
        val src = Array(64) { i -> NrDmrsMimoV90.Complex(if ((i and 1) == 0) 1.0 else -1.0, 0.0) }
        val tx = ofdmModulate(src)
        val channel = applyChannel(tx.time)
        val rx = ofdmDemodulate(channel.rx, 64, 4)
        val equalized = estimateAndEqualize(rx, src)
        val soft = softDemodulate(equalized.symbols)
        val ldpc = integrateLdpc()
        val checks = linkedMapOf(
            "V102" to (grid.collisions == 0 && grid.reserved.size == 36),
            "V103" to (tx.time.size == 68),
            "V104" to (equalized.symbols.size == 64),
            "V105" to (channel.rx.size == 68),
            "V106" to channel.snrDb.isFinite(),
            "V107" to equalized.symbols.all { it.re.isFinite() && it.im.isFinite() },
            "V108" to (equalized.method == "ZF"),
            "V109" to (soft.llr.size == 128 && soft.llr.all { it.isFinite() }),
            "V110" to (ldpc.encoded && ldpc.recovered && ldpc.decoded && ldpc.crcLengthOk)
        )
        return Report(checks.values.all { it }, checks, listOf(
            "V102 RE/resource reservation",
            "V103 numerology-independent OFDM primitive",
            "V104 MIMO processing boundary",
            "V105 deterministic AWGN channel",
            "V106 spatial-channel interface",
            "V107 DM-RS channel-estimation interface",
            "V108 ZF equalization interface",
            "V109 soft QAM LLR",
            "V110 V87/V84/V86 LDPC-rate-recovery-TB-CRC integration"
        ))
    }
}
