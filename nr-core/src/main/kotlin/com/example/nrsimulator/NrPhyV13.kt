...
/**
 * NR v13 additive end-to-end PDSCH reference link.
 *
...
data class NrPhyV13Config(
    ...
    val ptRsEnabled: Boolean = true,
    val ptRsTimeDensity: Int = 4,
    val ptRsFreqDensity: Int = 4,
    val cfoHz: Double = 250.0,
    val sfoPpm: Double = 2.0,
    val phaseNoiseStdRad: Double = 0.015,
    val seed: Int = 0x1301
)

data class NrPhyV13Result(
    val payloadBits: Int,
    val bg: Int,
    val zc: Int,
    ...
    val bler: Double,
    val pass: Boolean,
    val ptRsResources: Int,
    val residualPhaseDeg: Double,
    val cfoCorrectionPass: Boolean,
    val note: String
)

class NrPhyV13 {
    fun run(cfg: NrPhyV13Config): NrPhyV13Result {
        ...
        val ptRsPositions = ArrayList<Pair<Int, Int>>()
        if (cfg.ptRsEnabled) {
            val td = cfg.ptRsTimeDensity.coerceAtLeast(2)
            val fd = cfg.ptRsFreqDensity.coerceAtLeast(2)
            for (l in 0 until symbols) {
                if (l != dmrsL && l > dmrsL && ((l - dmrsL) % td == 0)) {
                    for (k0 in 0 until nSc step fd) if (!dmrsSet.contains(k0 to l)) ptRsPositions += k0 to l
                }
            }
        }
        val ptRsSet = ptRsPositions.toHashSet()
        val dataPositions = ArrayList<Pair<Int, Int>>()
        for (l in 0 until symbols) for (k0 in 0 until nSc) {
            if (!dmrsSet.contains(k0 to l) && !ptRsSet.contains(k0 to l)) dataPositions += k0 to l
        }
        ...
        if (cfg.ptRsEnabled) {
            val seq = ptRsSequence(ptRsPositions.size, 0x1300 + cfg.seed)
            for (i in ptRsPositions.indices) {
                val (k0, l) = ptRsPositions[i]
                txGrid[0][l][offset + k0] = seq[i]
            }
        }
        ...
        val received0 = channel.apply(ofdmFdTx, cfg.snrDb, 0x33)
        val impaired = applyImpairments(received0, cfg, nSc)
        val hEst = estimateChannel(impaired, dmrs, layers, rx, nSc, offset)
        val ptDiag = if (cfg.ptRsEnabled) estimatePtRsPhase(impaired, hEst, ptRsPositions, txGrid, layers, rx) else PhaseEstimate(0.0, false)
        val received = compensateDeterministicOffsets(impaired, cfg, nSc)
        val correction = PhaseCorrection(received, ptDiag.estimatedDeg, ptDiag.pass)
        ...
        return NrPhyV13Result(
            ...
            ptRsPositions.size, correction.residualPhaseDeg, correction.pass,
            "V13 extends V12 with a PT-RS resource path, deterministic CFO/SFO compensation, and phase-noise impairment injection. PT-RS provides a receiver-side common-phase diagnostic; CFO/SFO correction is deterministic in this reference layer. Full 38.211/38.214 PT-RS density/offset selection and closed-loop phase-noise tracking are reserved for later conformance work."
        )
    }
    ...
    private data class PhaseCorrection(val grid: Array<Array<Array<Complex>>>, val residualPhaseDeg: Double, val pass: Boolean)

    private fun applyImpairments(
        input: Array<Array<Array<Complex>>>, cfg: NrPhyV13Config, nSc: Int
    ): Array<Array<Array<Complex>>> {
        val out = Array(input.size) { Array(input[0].size) { Array(nSc) { Complex(0.0, 0.0) } } }
        val rng = Random(cfg.seed xor 0x5A17)
        var pn = 0.0
        val scsHz = cfg.scsKHz * 1000.0
        for (l in input[0].indices) {
            pn += gaussian(rng) * cfg.phaseNoiseStdRad
            for (k in 0 until nSc) {
                val cfoCycles = cfg.cfoHz / scsHz * l
                val sfoCycles = cfg.sfoPpm * 1e-6 * (k - nSc / 2.0) / nSc
                val phase = 2.0 * PI * (cfoCycles + sfoCycles) + pn
                val rot = Complex(kotlin.math.cos(phase), kotlin.math.sin(phase))
                for (r in input.indices) out[r][l][k] = input[r][l][k] * rot
            }
        }
        return out
    }

    private data class PhaseEstimate(val estimatedDeg: Double, val pass: Boolean)

    private fun estimatePtRsPhase(
        input: Array<Array<Array<Complex>>>, h: Array<Array<Complex>>,
        positions: List<Pair<Int, Int>>, txGrid: Array<Array<Array<Complex>>>, layers: Int, rx: Int
    ): PhaseEstimate {
        if (positions.isEmpty()) return PhaseEstimate(0.0, false)
        var s = 0.0; var c = 0.0; var n = 0
        for ((k, l) in positions) {
            val x = txGrid[0][l][k]
            for (r in 0 until rx) {
                val hh = h[k][r * layers]
                if (hh.abs2() < 1e-5 || x.abs2() < 1e-5) continue
                val ratio = divide(input[r][l][k], hh * x)
                val mag = kotlin.math.sqrt(ratio.abs2())
                if (mag < 0.35 || mag > 2.8) continue
                val a = kotlin.math.atan2(ratio.im, ratio.re)
                s += kotlin.math.sin(a); c += kotlin.math.cos(a); n++
            }
        }
        if (n == 0) return PhaseEstimate(180.0, false)
        val deg = kotlin.math.abs(kotlin.math.atan2(s / n, c / n)) * 180.0 / PI
        return PhaseEstimate(deg, deg < 15.0)
    }

    private fun compensateDeterministicOffsets(
        input: Array<Array<Array<Complex>>>, cfg: NrPhyV13Config, nSc: Int
    ): Array<Array<Array<Complex>>> {
        val out = Array(input.size) { r -> Array(input[0].size) { l -> input[r][l].clone() } }
        val scsHz = cfg.scsKHz * 1000.0
        for (l in input[0].indices) for (k in 0 until nSc) {
            val cfoCycles = cfg.cfoHz / scsHz * l
            val sfoCycles = cfg.sfoPpm * 1e-6 * (k - nSc / 2.0) / nSc
            val phase = 2.0 * PI * (cfoCycles + sfoCycles)
            val rot = Complex(kotlin.math.cos(-phase), kotlin.math.sin(-phase))
            for (r in out.indices) out[r][l][k] = out[r][l][k] * rot
        }
        return out
    }

    private fun ptRsSequence(n: Int, seed: Int): Array<Complex> {
        val rng = Random(seed)
        return Array(n) {
            val b0 = if (rng.nextBoolean()) 1.0 else -1.0
            val b1 = if (rng.nextBoolean()) 1.0 else -1.0
            Complex(b0, b1) * (1.0 / sqrt(2.0))
        }
    }
    ...
    private fun gaussian(rng: Random): Double {
        ...
        return sqrt(-2.0 * kotlin.math.ln(u)) * kotlin.math.cos(2.0 * PI * v)
    }
    ...
    private fun fail(cfg: NrPhyV13Config, payload: Int, why: String) = NrPhyV13Result(payload, 2, 0, 0, 0, 0, 0, 0, 0, cfg.txAntennas, cfg.rxAntennas, cfg.layers, 0, 0.0, 0.0, 0, false, false, 0.0, 0.0, 1.0, false, 0, 180.0, false, why)
}
