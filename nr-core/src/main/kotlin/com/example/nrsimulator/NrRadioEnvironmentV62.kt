...
data class NrUeStateV62(
    val ueId: Int, val xM: Double, val yM: Double, val distanceM: Double,
    val rsrpDbm: Double, val sinrDb: Double, val cqi: Int, val mcs: Int,
    val rank: Int, val allocatedPrbs: Int, val throughputMbps: Double, val bler: Double
)

data class NrRadioEnvironmentConfigV62(
    val ueCount: Int = 8, val cells: Int = 1, val prbs: Int = 52, val scsKHz: Int = 30,
    val carrierGHz: Double = 3.5, val txPowerDbm: Double = 46.0, val noiseFigureDb: Double = 7.0,
    val bandwidthMHz: Double = 0.0, val velocityKmh: Double = 30.0, val cellRadiusM: Double = 250.0,
    val txAntennas: Int = 4, val rxAntennas: Int = 4, val layers: Int = 2, val shadowingDb: Double = 4.0,
    val trafficBufferMb: Double = 20.0, val slotIndex: Long = 0L, val seed: Int = 6201
)

data class NrRadioEnvironmentResultV62(
    val slotIndex: Long, val cellCount: Int, val ueStates: List<NrUeStateV62>,
    val totalThroughputMbps: Double, val meanSinrDb: Double, val meanCqi: Double,
    val spectralEfficiencyBpsHz: Double, val utilizationPercent: Double, val fairness: Double,
    val stageSummary: List<String>
)

/** Additive V62 system-level radio environment. V1-V61 are deliberately untouched. */
object NrRadioEnvironmentV62 {
    private val mcsEfficiency = doubleArrayOf(
        .234, .377, .601, .877, 1.175, 1.477, 1.914, 2.406, 2.730, 3.322, 3.902, 4.523, 5.115, 5.555,
        6.226, 6.914, 7.406, 7.914, 8.453, 8.914, 9.450, 10.029, 10.626, 11.115, 11.830, 12.500, 13.225, 13.926
    )
    ...
    private fun gaussian(r: Random): Double {
        var u1 = r.nextDouble()
        while (u1 <= 0.0) u1 = r.nextDouble()
        val u2 = r.nextDouble()
        return sqrt(-2.0 * ln(u1)) * cos(2.0 * Math.PI * u2)
    }

    private fun cqiFromSinr(sinrDb: Double): Int = when {
        sinrDb < -6.7 -> 0; sinrDb < -4.7 -> 1; sinrDb < -2.3 -> 2; sinrDb < 0.2 -> 3
        sinrDb < 2.4 -> 4; sinrDb < 4.3 -> 5; sinrDb < 6.3 -> 6; sinrDb < 8.4 -> 7
        sinrDb < 10.3 -> 8; sinrDb < 12.3 -> 9; sinrDb < 14.3 -> 10; sinrDb < 16.3 -> 11
        else -> 12
    }
    ...
    private fun rank(tx: Int, rx: Int, requested: Int, sinrDb: Double): Int {
        ...
        return when { sinrDb < 5.0 -> 1; sinrDb < 12.0 -> minOf(limit, 2); else -> limit }
    }
    ...
    private fun jain(values: List<Double>): Double {
        val sum = values.sum(); val sq = values.sumOf { it * it }
    ...
    fun run(c0: NrRadioEnvironmentConfigV62 = NrRadioEnvironmentConfigV62()): NrRadioEnvironmentResultV62 {
        val c = c0.copy(
            ueCount = c0.ueCount.coerceIn(1, 64), cells = c0.cells.coerceIn(1, 7), prbs = c0.prbs.coerceIn(1, 106),
            scsKHz = if (c0.scsKHz in listOf(15, 30, 60)) c0.scsKHz else 30,
            carrierGHz = c0.carrierGHz.coerceIn(0.5, 100.0), txAntennas = c0.txAntennas.coerceIn(1, 8),
            rxAntennas = c0.rxAntennas.coerceIn(1, 8), layers = c0.layers.coerceIn(1, minOf(c0.txAntennas, c0.rxAntennas)),
            velocityKmh = c0.velocityKmh.coerceIn(0.0, 500.0), cellRadiusM = c0.cellRadiusM.coerceIn(25.0, 5000.0)
        )
        ...
        val raw = (0 until c.ueCount).map { id ->
            ...
            val x = radial * cos(angle) + travel; val y = radial * sin(angle)
            val d = hypot(x, y).coerceAtMost(c.cellRadiusM * 1.35)
            val pl = pathLossDb(d, c.carrierGHz)
            val shadow = if (c.shadowingDb == 0.0) 0.0 else gaussian(r) * c.shadowingDb
            ...
            Raw(id + 1, x, y, d, rsrp - noiseDbm - interCellDb - dopplerPenalty, rsrp)
        }
        ...
        val weights = raw.associate { u -> u.id to (mcsEfficiency[mcs.getValue(u.id)] * ranks.getValue(u.id) + 0.25) / (1.0 + 0.12 * u.id) }
        val totalWeight = weights.values.sum().coerceAtLeast(1e-9)
        val allocated = raw.associate { u -> u.id to maxOf(0, floor(c.prbs * weights.getValue(u.id) / totalWeight).toInt()) }.toMutableMap()
        var left = c.prbs - allocated.values.sum(); val order = raw.sortedByDescending { weights.getValue(it.id) / maxOf(1, allocated.getValue(it.id)) }; var k = 0
        while (left > 0) { val id = order[k % order.size].id; allocated[id] = allocated.getValue(id) + 1; left--; k++ }
        stage += "Scheduler: proportional-fair weighted PRB allocation"
        stage += "Link adaptation: SINR -> CQI -> MCS -> rank"

        val states = raw.map { u ->
            val q = cqi.getValue(u.id); val m = mcs.getValue(u.id); val rk = ranks.getValue(u.id); val prb = allocated.getValue(u.id)
            val rawMbps = prb * 12.0 * 14.0 * c.scsKHz * 1000.0 * mcsEfficiency[m] * rk / 1e6
        ...
        val throughputs = states.map { it.throughputMbps }; val total = throughputs.sum(); val occupied = states.sumOf { it.allocatedPrbs }
        val meanSinr = states.map { it.sinrDb }.average(); val meanCqi = states.map { it.cqi }.average()
        val spectral = if (bwHz > 0.0) total * 1e6 / bwHz else 0.0; val fairness = jain(throughputs)
