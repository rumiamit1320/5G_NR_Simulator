...
object NrIntegratedLinkV61 {
 ...
 fun run(c0:NrIntegratedLinkConfigV61=NrIntegratedLinkConfigV61()):NrIntegratedLinkResultV61{
  ...
  val r=Random(cfg.seed);val s=mutableListOf<String>();val payload=Dsp.bits(cfg.payloadBits,r);s+="Bits: ${payload.size}";val tb=appendCrc(payload);s+="CRC-24C: ${tb.size}";val scr=scramble(tb,cfg.seed xor 0x61);s+="Scrambling: ${scr.size}";val n=generateSequence(32){it*2}.first{it>=scr.size};val cw=NrPolarV47.encode(scr,n);s+="Polar encode: K=${scr.size}, N=$n";val rm=NrPolarV47.rateMatch(cw,n,cfg.rv);s+="Rate matching: E=${rm.size}, RV=${cfg.rv}"
  val bps=log2(m.toDouble()).roundToInt();val padded=rm.copyOf(((rm.size+bps-1)/bps)*bps);val q=Dsp.qam(padded,m);s+="QAM: ${m}-QAM, symbols=${q.size}";s+="Layer mapping: ${layers} logical layer(s)";val precoded=q.copyOf();s+="Precoding: ${tx} Tx / ${layers} layer(s)";val td=ifft(precoded);s+="OFDM: ${N}-point IFFT";val noisy=Dsp.addAwgn(td,cfg.snrDb,r);val fd=fft(noisy,precoded.size);s+="Channel/noise: AWGN ${cfg.snrDb} dB";val snrLin=10.0.pow(cfg.snrDb/10.0);val gain=if(cfg.equalizer==NrLinkEqualizerV61.MMSE)snrLin/(snrLin+1.0)else 1.0;val eq=Array(fd.size){fd[it]*(1.0/gain)};s+="Equalizer: ${cfg.equalizer}"
  val evm=Dsp.evm(precoded,eq);val hard=demod(eq,m).copyOf(rm.size);s+="Demodulation: ${hard.size} bits";val recovered=NrPolarV47.rateRecover(hard,n,cfg.rv);val decoded=NrPolarV47.decode(recovered,scr.size);val descr=scramble(decoded,cfg.seed xor 0x61);s+="Polar decode: ${descr.size} bits";val data=descr.copyOf(minOf(payload.size,descr.size));var errors=abs(payload.size-data.size);for(i in data.indices)if(payload[i]!=data[i])errors++;val crc=descr.size>=tb.size&&crcOk(descr.copyOf(tb.size));val ber=errors.toDouble()/payload.size;s+="CRC: ${if(crc)"PASS" else "FAIL"}"
  val thr=cfg.prbs*12*14*cfg.scsKHz*1000.0*log2(m.toDouble())*cfg.codeRate*layers*.83/1e6;s+="Metrics: BER=${"%.6g".format(ber)}, EVM=${"%.3f".format(evm)}%";return NrIntegratedLinkResultV61(payload.size,cw.size,rm.size,data.size,errors,crc,ber,evm,thr,q.size,layers,tx,rx,N,precoded.size,s,q.take(96).toTypedArray(),eq.take(96).toTypedArray(),eq.copyOf())
 }
}
