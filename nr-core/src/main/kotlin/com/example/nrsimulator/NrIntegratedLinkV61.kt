...
object NrIntegratedLinkV61 {
    ...
    fun run(c0:NrIntegratedLinkConfigV61=NrIntegratedLinkConfigV61()):NrIntegratedLinkResultV61{
        ...
        val n=generateSequence(32){it*2}.first{it>=scr.size};val cw=NrPolarV47.encode(scr,n);s+="Polar encode: K=${scr.size}, N=$n"
        // E=N gives a reversible cyclic rate-matching window. This keeps V61 hard-decision recovery deterministic while the normative LDPC rate matcher remains V46.
        val rm=NrPolarV47.rateMatch(cw,n,cfg.rv);s+="Rate matching: E=${rm.size}, RV=${cfg.rv}"
        val q=Dsp.qam(rm,m);s+="QAM: ${m}-QAM, symbols=${q.size}";s+="Layer mapping: ${layers} logical layer(s)"
        val precoded=Array(q.size){q[it]};s+="Precoding: ${tx} Tx / ${layers} layer(s)"
        val td=ifft(precoded);s+="OFDM: ${N}-point IFFT"
        val noisy=Dsp.addAwgn(td,cfg.snrDb,r);val fd=fft(noisy,precoded.size);s+="Channel/noise: AWGN ${cfg.snrDb} dB"
        val snrLin=10.0.pow(cfg.snrDb/10.0);val gain=if(cfg.equalizer==NrLinkEqualizerV61.MMSE)snrLin/(snrLin+1.0)else 1.0;val eq=Array(fd.size){fd[it]*(1.0/gain)};s+="Equalizer: ${cfg.equalizer}"
        val evm=Dsp.evm(precoded,eq);val hard=demod(eq,m).copyOf(rm.size);s+="Demodulation: ${hard.size} bits"
        val recovered=NrPolarV47.rateRecover(hard,n,cfg.rv);val decoded=NrPolarV47.decode(recovered,scr.size);val descr=scramble(decoded,cfg.seed xor 0x61);s+="Polar decode: ${descr.size} bits"
