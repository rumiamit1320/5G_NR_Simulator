...
import json,sys
obj=json.loads(sys.argv[1]); cur=obj
...
# Syntax-check all additive V64 browser layers. Existing application JS is also checked
# where it is present, but no source is modified by this script.
...
for v in V19 V20 V21 V22 V23 V24 V25 V26 V27 V28 V29 V30 V31 V32 V33 V34 V35 V36 V37 V38 V39 V40 V41 V42 V43 V44 V45 V46 V47 V48 V49 V50 V51 V52 V53 V54 V55 V56 V57 V58 V59 V60 V63; do
  if out=$(api "version=$v"); then
    if grep -Fq "\"version\":\"$v\"" <<<"$out"; then pass "API version $v"; else fail "API version $v response version"; fi
  else fail "API version $v reachable"; fi
done

# V61 end-to-end API and V64 closed-loop API.
...
check "V64 closed-loop API" "$BASE_URL/api/lab?version=V64&slots=3&ue=4&cells=2&prbs=24&scs=30&velocity=60" '"version":"V64"'

# V63/V64 structural invariants. These are engineering invariants, not conformance claims.
...
for u in x['ueStates']:
    assert 1 <= int(u['cqi']) <= 15, u
    assert 0 <= int(u['allocatedPrbs']) <= 24, u
...
out=$(api 'version=V64&slots=4&ue=6&cells=2&prbs=36&scs=30&velocity=90')
python3 - "$out" <<'PY'
import json,sys,math
x=json.loads(sys.argv[1]); assert x.get('ok') is True; us=x.get('ueStates',[]); assert len(us)==6
for u in us:
    assert 1 <= int(u['cqi']) <= 15
    assert int(u['allocatedPrbs']) >= 0
    assert int(u.get('mcs',0)) >= 0
    assert math.isfinite(float(u['throughputMbps']))
print('V64 closed-loop invariants OK')
PY
pass "V64 closed-loop UE/range/finite-value invariants"

# Parameter-domain verification. The simulator supports the NR SCS values below;
# the checks intentionally do not claim that every simulator PRB setting is universal NR.
...
for mod in 'QPSK' '16-QAM' '64-QAM' '256-QAM'; do
  # Modulation is exercised through the V61 endpoint; the V63 bridge derives it from MCS.
...
# Negative tests: invalid/unsafe parameter values must not crash the service.
if curl -sS --max-time 30 -o /tmp/v64_negative.json -w '%{http_code}' "$BASE_URL/api/lab?version=V64&slots=0&ue=0&cells=0&prbs=0&scs=999&velocity=-100" | grep -Eq '^[2345][0-9][0-9]$'; then pass "Invalid V64 parameter request handled"; else fail "Invalid V64 parameter request handled"; fi

# UI feature presence: all seven V64 tabs and key controls must remain wired in radio.html.
...
# Standards mapping is deliberately conservative: this harness verifies consistency of
# implemented domains/relationships and labels anything requiring normative vectors as a
# model-consistency check rather than declaring full 3GPP conformance.
